...
describe("rawSearch", () => {
  it("calls memorySearch with query and returns results", async () => {
    ...
    const runner = createMockRunner(items);
...
describe("search", () => {
  it("preprocesses query before searching", async () => {
    const runner = createMockRunner([]);
    const inferFn = createMockInferFn("Rewritten query about diet");
    const results = await search("Oh sorry I like vegan food", 5, undefined, runner, inferFn);
    expect(results).toEqual([]);
  });
  ...
  it("falls back to raw query if preprocessing fails", async () => {
    ...
    const results = await search("raw query here", 5, undefined, runner, inferFn);
    expect(results).toEqual([]);
  });
});
